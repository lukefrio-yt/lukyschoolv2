...
onAct('t-cls-create-prompt', () => {
  ...
  openModal(
    ...
    '<form data-form="t-cls-create">' +
      '<div class="field"><label>Název třídy</label><input name="name" maxlength="30" placeholder="Např. 1. A" required autofocus></div>' +
...
function tPrehled() {
  ...
  '<div class="grid grid-2" style="margin-top:16px">' +
    ...
    '</div>' +
    '<div>' +
      '<div class="card"><div class="card-title" style="color:var(--bad)">' + ic('alert', 17) + ' Ohrožení žáci</div>' +
        (risk.length
...
function atLegend() {
  ...
  '<div class="small-note" style="margin:-4px 0 12px">' +
    'Legenda ke štítkům: <b style="color:var(--ok)">✓</b> Rodič dítě omluvil, ' +
    '<b style="color:var(--bad)">✗</b> Rodič dítě neomluvil.</div>';
}
function tDochazka() {
  clearTick();
  if (!myClasses().length) return noClassPrompt();
  ...
  return '' +
  '<div class="page-head"><div><h1>Docházka</h1>' +
  ...
  (sts.length
    ? (isAppMode()
      /* mobil: přehledné karty místo široké tabulky */
      ? '<div class="card" style="margin-top:16px"><div class="card-title">' + ic('calendar', 16) + ' Absence žáků</div>' +
        '<div class="mob-abs-list">' + rows.map(({ s, ov }) => {
          const t = ov.total;
          return '<div class="mob-abs' + (isAtRisk(s.id) ? ' risk' : '') + '">' +
            '<div class="mob-abs-head">' + teacherAva(s, 34) +
              '<b>' + escapeHtml(s.first + ' ' + s.last) + '</b>' +
              (t.missing
                ? '<span class="chip ' + (t.N ? 'chip-bad' : 'chip-warn') + '" style="margin-left:auto">' + t.missing + '/' + t.lessons + '</span>'
                : '<span class="chip chip-ok" style="margin-left:auto">Bez absence</span>') + '</div>' +
            (t.missing
              ? '<div class="mob-abs-break">Omluveno <b style="color:var(--ok)">' + t.A + '</b> · Čeká <b style="color:var(--warn)">' + t.C + '</b> · Neomluveno <b style="color:var(--bad)">' + t.N + '</b></div>'
              : '') +
          '</div>';
        }).join('') + '</div>' + atLegend() + '</div>'
...
function tKlasifikace() {
  ...
  return '' +
  '<div class="page-head"><div><h1>Známkování</h1><div class="sub"></div></div>' +
    '<div class="page-acts">' +
    ...
    ? '<div class="card"><div class="tbl-wrap" style="max-height:520px;overflow:auto">' +
        '<table class="tbl" style="min-width:900px"><thead><tr>' +
          ...
          (cols.length ? '<tr><td style="position:sticky;left:0;background:var(--surface-2)"><b>Ø třídy</b></td>' +
            ...
            '<td></td><td></td></tr>' : '') +
        '</tbody></table></div>' +
        '<div class="small-note" style="margin-top:10px">Legenda: N = Nepsal test, ale bude dopisovat | A = Nepsal test a nebude dopisovat | ? = Nadcházející test.</div>' +
      '</div>'
...
onAct('form:t-col-save', f => {
  ...
  if (!col) return;
  const fd = new FormData(f);
  col.title = String(fd.get('title')).trim().slice(0, 60);
...
function tKniha() {
  ...
  const secZapisy =
    ...
    '</div>';
  const head = '' +
    '<div class="page-head"><div><h1>Třídní kniha</h1><div class="sub">Třídní kniha | Úkol se žákům propíše do kategorie: Moje úkoly</div></div></div>' +
    clsScopePills();
  if (isAppMode()) {
...
function cbDropPend() { Object.keys(CB_PEND).forEach(k => delete CB_PEND[k]); }  const ATT_TXT = {
  '/': ['Nezadáno', 'var(--muted)'],
  P: ['Přítomen', 'var(--ok)'],
...
function cbAttendanceCard(cid, date, period, subjAuto) {
  ...
  return '<div class="card" style="margin-top:16px">' +
    ...
    '<div class="small-note" style="margin-top:10px">Legenda ke štítkům: <b style="color:var(--ok)">✓</b> Rodič dítě omluvil, <b style="color:var(--bad)">✗</b> Rodič dítě neomluvil.</div>' +
  '</div>';
}
onAct('cb-att:', el => {
  const cid = activeClsId();
  ...
  const secNova =
    '<div class="card">' +
      ...
      '<div style="display:flex;gap:10px;flex-wrap:wrap">' +
        ...
        '<button class="btn btn-ghost btn-sm" data-act="t-msg-later">' + ic('clock', 14) + ' Naplánovat</button>' +
      '</div>' +
    '</div>';
  const secKonv =
...
onAct('t-msg-later', () => {
  const ta = document.getElementById('tmsg-text');
  if (!ta || !ta.value.trim()) { toast('Napište text zprávy', 'warn'); return; }
  /* Naplánování na konkrétní datum a čas (nelze do minulosti) */
  const now = new Date();
  const pad = n => String(n).padStart(2, '0');
  const minDt = now.getFullYear() + '-' + pad(now.getMonth() + 1) + '-' + pad(now.getDate());
  ...
  openModal(
    ...
    '<form data-form="t-msg-later-save">' +
      ...
      '<button class="btn btn-primary">' + ic('clock', 14) + ' Naplánovat</button>' +
    '</form>');
});
onAct('form:t-msg-later-save', f => {
  const fd = new FormData(f);
  const ta = document.getElementById('tmsg-text');
  const date = String(fd.get('date') || '');
  const time = String(fd.get('time') || '');
  if (!ta || !ta.value.trim()) { closeModal(); toast('Napište text zprávy', 'warn'); return; }
  const t = new Date(date + 'T' + (time || '08:00') + ':00');
  if (!t.getTime()) { toast('Zadejte platné datum a čas', 'bad'); return; }
  if (t.getTime() <= Date.now()) { toast('Nelze naplánovat do minulosti', 'bad'); return; }
  ...
  closeModal();
  ta.value = '';
  toast('Naplánováno na ' + fmtDate(date) + ' ' + time + ' ✓', 'ok');
});
...
function tOmluvenky() {
  ...
  return '' +    '<div class="page-head"><div><h1>Schvalování omluvenek</h1><div class="sub">Jedním kliknutím schválíte / zamítnete | Zamítnutí se počítá jako neomluvená hodina</div></div></div>' +
  clsScopePills() +
  '<div class="grid grid-2">' +
    ...
    '<div>' +
      ...
      '<div class="card" style="margin-top:16px"><div class="card-title">' + ic('alert', 16) + ' Absence (' + escapeHtml(classOf(cid).name) + ')' +
        (!isAppMode() ? '<span style="margin-left:auto;font-size:11.5px;color:var(--muted);font-weight:600">Limit: 25% zameškaných hodin</span>' : '') + '</div>' +
        (absRows.length
...
function rzModeTabs(cid) {
  ...
  const tabs = [
    { k: 'den', label: 'Můj den' },
    { k: 'editor', label: canEdit ? (isAppMode() ? 'Nastavit rozvrh 🖥️' : 'Nastavit rozvrh') : 'Rozvrh třídy' }
  ];
...
function tRozvrh() {
  ...
  const cid = activeClsId();
  const cls = classOf(cid);
  let mode = localStorage.getItem('t_roz_mode') || 'den';
  /* Nastavit rozvrh jen na počítači */
  const editorBlocked = isAppMode() && mode === 'editor';
  if (editorBlocked) mode = 'den';
  return '' +
  ...
  (editorBlocked
    ? '<div class="card" style="border-color:var(--warn);margin-bottom:14px"><b>' + ic('lock', 15) + ' Pouze na počítači</b><p class="small-note" style="margin:6px 0 0">Nastavování rozvrhu je dostupné jen na počítači 🖥️</p></div>' + rzDenHtml(cid)
...
onAct('sub-edit:', el => {
  ...
  openModal('<h3>Upravit předmět „' + escapeHtml(s.name) + '“</h3>' +
    '<form data-form="sub-edit">' +
      ...
      '<div class="field"><label>Název</label><input name="name" maxlength="40" value="' + escapeHtml(s.name) + '" required></div>' +
...
function tUcebny() {
  ...
  const row = r => {
    ...
    return '<div class="list-row">' +
      ...
      '<div class="row-sub">Zkratka v rozvrhu: <code class="mono">' + escapeHtml(short) + '</code>' + (n ? ' · Použita v rozvrhu na ' + n + ' místech' : '') + '</div></div>' +
      '<div style="display:flex;gap:6px">' +
...
onAct('rm-edit:', el => {
  ...
  openModal('<h3>Upravit učebnu „' + escapeHtml(r.name) + '“</h3>' +
    '<form data-form="rm-edit">' +
      ...
      '<div class="field"><label>Název</label><input name="name" maxlength="40" value="' + escapeHtml(r.name) + '" required></div>' +
      ...
      ? '<div class="list">' + rows.map(r =>
          '<div class="mob-cred">' +
            '<div class="mob-cred-head"><span class="ava">' + escapeHtml(r.s.first.charAt(0)) + '</span>' +
              '<b>' + escapeHtml(r.s.first + ' ' + r.s.last) + '</b></div>' +
            '<div class="mob-cred-logins">' +
              (r.acc ? '<div class="mob-cred-line"><span>Login</span><code class="mono">' + escapeHtml(r.acc.username) + '</code></div>' : '') +
              (r.par ? '<div class="mob-cred-line"><span>Rodič</span><code class="mono">' + escapeHtml(r.par.username) + '</code></div>' : '') +
            '</div>' +
...
onAct('t-pass-reset:', el => {
  ...
  const np = genPassword(target.role);
...
function tPoznamky() {
  ...
  '<div class="page-head"><div><h1>Výchovná opatření</h1>' +
    '<div class="sub">' + escapeHtml(cls.name) + ' | Výchovná opatření - Poznámky, pochvaly, důtky, napomenutí a další...</div></div>' +
...
function tPlanAkci() {
  ...
  '<div class="page-head"><div><h1>Plán akcí</h1>' +
    '<div class="sub">' + escapeHtml(cls.name) + ' | Budoucí akce třídy - Žáci i rodiče vidí, co je čeká</div></div>' +
